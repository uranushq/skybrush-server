...
# Graded downwash exposure — OFF unless a caller asks for it (a zero budget
# reproduces the all-or-nothing rule above bit for bit). It exists as a
# second-tier fallback for formations too tight to fly into any other way:
#
#   |dz| <  separation + DOWNWASH_HARD_FLOOR        forbidden at any duration
#   |dz| <  separation + DOWNWASH_ROUTE_CLEARANCE   allowed while the time
#                                                   spent there stays under
#                                                   the budget
#   otherwise                                       unrestricted
#
# The clock only runs against PARKED drones — the only obstacles a route is
# planned around. Drones holding formation while translating together never
# close on each other, and the safety of that geometry is the formation's own
# vertical separation, not a transit rule.
#
# HARD_MIN_SEPARATION is the floor the whole planner is built on ("a request
# may RAISE the separation but nothing may ever lower it below this"), so the
# graded band bottoms out there rather than at a fresh invented number.
...
class PathSolver:
    ...
    def __init__(
        ...
        # Consecutive steps a route may spend in the graded downwash band.
        # Zero — the default, and whatever a caller passes when a single step
        # already lasts longer than the whole budget — restores the old
        # all-or-nothing rule exactly.
    ...
    def _plan_route(
        ...
        for obs in statics:
            if envelope_overlap(
                ...
                return None
            # With a budget, a start inside the *graded* band is routable —
            # the exposure accounting below lets it step out. Only the hard
            # floor is hopeless: every outgoing edge starts blocked there and
            # no budget can buy one.
            start_pad = (
                DOWNWASH_HARD_FLOOR
        ...
        # already spent in the graded band, so no path can accumulate more
        # exposure than the budget allows. With a zero budget the extra
        # component never leaves 0 and the search is the old one.
        ...
        while open_heap and expansions < ASTAR_MAX_EXPANSIONS:
            ...
            for ox, oy, oz in _DETOUR_OFFSETS:
                ...
                # this edge produces. Prune before touching geometry.
